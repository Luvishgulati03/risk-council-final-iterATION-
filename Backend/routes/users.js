...
router.get('/', authRequired, adminOnly, async (req, res) => {
    try {
        const [rows] = await db.query(
            'SELECT id, name, email, role, approval_status, organization_name, gst, pan, incorporation_number, phone, is_banned, created_at FROM users ORDER BY created_at DESC'
        );
        res.json(rows);
    } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
...
router.patch('/:id/role', authRequired, adminOnly, async (req, res) => {
    const { role } = req.body;
    if (!['user', 'member', 'admin', 'executive', 'university', 'company'].includes(role)) {
...
router.post('/', authRequired, adminOnly, async (req, res) => {
    ...
    try {
        ...
        const assignedRole = ['user', 'member', 'university', 'company', 'admin', 'executive'].includes(role) ? role : 'user';

        const [result] = await db.query(
            `INSERT INTO users (name, email, password_hash, role, approval_status, organization_name, gst, pan, incorporation_number, phone)
             VALUES (?, ?, ?, ?, 'approved', ?, ?, ?, ?, ?)`, // Admin created users are auto-approved
...
// GET /api/users/me/profile — get own full profile
router.get('/me/profile', authRequired, async (req, res) => {
    try {
        const [rows] = await db.query(
            'SELECT id, name, email, role, approval_status, organization_name, phone, bio, linkedin_url, twitter_url, website_url, profile_image, created_at FROM users WHERE id = ?',
            [req.user.id]
        );
...
router.put('/me/profile', authRequired, profileUpload.single('profile_image'), async (req, res) => {
    ...
    try {
        ...
        if (imagePath) { updates.push('profile_image = ?'); values.push(imagePath); }
